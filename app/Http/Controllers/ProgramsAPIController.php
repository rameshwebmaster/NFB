...
class ProgramsAPIController extends Controller
{
    ...
    public function getNutritionProgram(Request $request)
    {
        ...
        $program->sections->each(function ($section) {
            $section->addTranslationItem('program_section_title', $section->trans('section_title') ?? "");
            $section->entries->each(function ($entry) {
                $entry->addTranslationItem('program_entry_title', $entry->trans('program_entry_title') ?? "");
                $entry->addTranslationItem('program_entry_quantity', $entry->trans('program_entry_quantity') ?? "");
            });
        });
    ...
    public function getExerciseProgram()
    {
        ...
        if ($category) {
            ...
            $category->addTranslationItem('category_description', $category->trans('category_description') ?? "");
            return $category;
        }
